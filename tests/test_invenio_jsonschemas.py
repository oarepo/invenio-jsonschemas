...
def build_schemas(id):
    ...
    return {
        "rootschema_{}.json".format(id): schema_template.format(
            "rootschema_{}".format(id)
        ),
        "sub1/subschema_{}.json".format(id): schema_template.format(
            "subschema_1_{}".format(id)
        ),
        "sub2/subschema_{}.json".format(id): schema_template.format(
            "subschema_2_{}".format(id)
        ),
        "sub3/subschema_{}.json".format(id): schema_template.format(
            "subschema_3_{}".format(id)
        ),
    }
...
def test_replace_refs_in_view(app, pkg_factory, mock_entry_points):
    ...
    with pkg_factory(schemas) as pkg1:
        ...
        with app.test_client() as client:
            ...
            assert res.status_code == 200
            assert json.loads(schemas["root.json"]) == json.loads(
            ...
            res = client.get("{0}/{1}".format(endpoint, "root.json"))
            assert res.status_code == 200
            assert json.loads(schemas["sub/schema.json"]) == json.loads(
                res.get_data(as_text=True)
            )
...
def test_replace_resolve_in_view(app, pkg_factory, mock_entry_points):
    ...
    with pkg_factory(schemas) as pkg1:
        ...
        with app.test_client() as client:
            ...
            res = client.get("{0}/{1}?resolved=1".format(endpoint, "root.json"))
            assert res.status_code == 200
            assert json.loads(schemas["sub/schema.json"]) == json.loads(
                res.get_data(as_text=True)
            )
...
def test_alternative_entry_point_group_init(app, pkg_factory, mock_entry_points):
    ...
    entry_point_group = "invenio_jsonschema_test_entry_point"
    with pkg_factory(schema_files_1) as pkg1, pkg_factory(schema_files_2) as pkg2:
        mock_entry_points.add(entry_point_group, "entry1", pkg1)
        mock_entry_points.add(entry_point_group, "entry2", pkg2)
...
@mock.patch(
    "invenio_jsonschemas.ext.InvenioJSONSchemasState.get_schema", mock_get_schema
)
def test_jsonresolver():
    ...
    with app.app_context():
        json_resolver = JSONResolver(
            plugins=[
                "invenio_jsonschemas.jsonresolver",
            ]
        )
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate(
                {"foo": "foo_value", "bar": "not_an_int"}, schema, resolver=resolver
            )
        assert exc_info.value.schema == {"type": "integer"}


@pytest.mark.parametrize("url_scheme", [None, "http", "https"])
def test_url_mapping(app, dir_factory, url_scheme):
    ...
    with dir_factory(schema_files) as directory:
        ...
        with app.app_context():
            ...
            assert (
                ext.url_to_path(
                    "{0}://inveniosoftware.org/schemas/invalid.json".format(url_scheme)
                )
                is None
            )
            assert (
                ext.url_to_path(
                    "{0}://example.org/schemas/sub1/subschema_1.json".format(url_scheme)
                )
                is None
            )
...
def test_whitelisting_entry_points(
    app, pkg_factory, mock_entry_points, whitelisted, expected
):
    ...
    entry_point_group = "invenio_jsonschema_test_entry_point"
    with pkg_factory(entries["entry1"]) as pkg1, pkg_factory(entries["entry2"]) as pkg2:
        mock_entry_points.add(entry_point_group, "entry1", pkg1)
        mock_entry_points.add(entry_point_group, "entry2", pkg2)
        ext = InvenioJSONSchemas()
        ext = ext.init_app(app, entry_point_group=entry_point_group)
        ...
        "allOf": [
            {"type": "object", "properties": {"species": {"type": "string"}}},
            {
                "allOf": [
                    {
                        "type": "object",
                        "properties": {"species_nested": {"type": "string"}},
                    }
                ]
            },
        ],
